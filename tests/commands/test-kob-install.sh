...
function kob_init {
  ...
  KOBMAN_DIR="/home/user/.kobman"
  
  #TODO:- Set path_to_kob_env_tests(e.g - src/tests/envs), kob_environment_under_test, kob_test_namespace.
  path_to_kob_env_tests="${KOBMAN_DIR}/env"
  kob_environment_under_test=$1 
  kob_test_namespace=$2
  ...
  if [ $? -eq 0 ];
        then echo "kob is installed" 
  else
        echo "kob not installed.."
  fi
  ...
  function _test_kob_env {
    source "$path_to_kob_env_tests/test-${kob_environment_under_test}.sh"
  }
}
